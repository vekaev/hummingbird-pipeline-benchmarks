...
const TABLES = [
  ...
  { id: 'T36', file: 'results/measured.md', heading: 'A third encode, and the order is backwards', title: 'Encode order on large sources',
    note: 'A resize step sits between the other two encodes and also passes no codec settings, so it runs at the defaults too. It does nothing below a resolution threshold, which is why it never appeared in the benchmark outputs -- but above it, the slowest highest-quality encode runs FIRST at full source resolution, immediately before the step that discards three quarters of those pixels. Read the size column: the fused arm is largest because it encodes once and stops, so it is both far faster and one generation-loss step shorter than what ships. Scope: the source was synthesised by upscaling, so the saving is a lower bound, and the percentages must NOT be applied to the stage cost, which also includes fetching.' },
  { id: 'T37', file: 'results/measured.md', heading: 'Replicated on different hardware', title: 'Encode order, replicated',
    note: 'The same benchmark on a different CPU architecture, a different encoder version and a different source clip. Absolute times differ; the ratios agree within about three percentage points. Contrast with the fidelity replication elsewhere in this pack, which reproduced the ordering and decomposition but landed 20-25% lower on magnitude -- which is why that claim is quoted loosely and this one tightly. A ratio between two encodes of the same content travels; an absolute measure against a reference does not.' },
  { id: 'T38', file: 'results/reporting.md', heading: 'Configuration key', title: 'Configuration key',
