...
| 22 | Ran the two kept changes together | They target different stages, so they ought to add — but two earlier attempts at this test were void: one passed flags to an image built before either change existed, the other set the cache to a size that misses on every frame. | -23.13% together, and the renderer change REPLICATED at -14.19% against -14.30% measured separately on a different image. Predicting the combined arm from the two separate experiments lands 2.25s off, inside the run-to-run spread, so the savings add. Each change moves only its own stage |
